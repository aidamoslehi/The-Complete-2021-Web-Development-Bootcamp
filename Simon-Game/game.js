...
$(".btn").on("click", function () {
  ...
  // 12.Add the contents of the variable userChosenColour created in step 10 to the end of this new userClickedPattern
  userClickedPattern.push(userChosenColour);
  // console.log(userClickedPattern);
  // 15.In the same way we played sound in nextSequence() , when a user clicks on a button, the corresponding sound should be played. e.g if the Green button is clicked, then green.mp3 should be played
...
function nextSequence() {
  ...
  // 6. Use jQuery to select the button with the same id as the randomChosenColour
  // 7.use jQuery to animate a flash to the button selected
  $("#" + randomChosenColour)
    .fadeIn(100)
    .fadeOut(100)
    .fadeIn(100);
...
function animatePress(currentColor) {
  ...
  //18.use Javascript to remove the pressed class after a 100 milliseconds.
  setTimeout(() => {
...
function playSound(name) {
  // 14.Take the code we used to play sound in the nextSequence() function and move it to playSound().

  // 8.use Javascript to play the sound for the button colour selected
...
function startOver() {
  // 34. Inside this function, you'll need to reset the values of level, gamePattern and started variables.Will be called at the "wrong" if statement.
  level = 0;
  gamePattern = [];
  started = false;
}
